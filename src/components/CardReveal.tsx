...
export const CardReveal = ({ cardId, onClose, isNew = true }: CardRevealProps) => {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="text-center w-full max-w-[340px] px-4 flex flex-col items-center">
          <motion.div
            ...
            >
              ...
              <div className="w-full h-full p-4 parchment-bg flex flex-col relative m-0.5 overflow-hidden">
                ...
                <div className="relative z-10 flex-1 flex flex-col text-left h-full">
                  ...
                  {/* Header Area */}
                  <div className="w-full flex flex-col mb-2 border-b border-black/10 pb-2">
                    <h3 className="text-xl font-hearth font-black text-[#3d2b1f] uppercase tracking-wider drop-shadow-sm leading-none truncate">{card.name}</h3>
                    <p className="text-[10px] text-[#8b4513] uppercase font-black tracking-[0.2em] mt-1 engraved">{card.rarity}</p>
                  </div>
                  ...
                  {/* Main Info Area */}
                  <div className="flex-1 flex flex-col w-full min-h-0 space-y-2">
                    <div className="space-y-0.5 shrink-0">
                      <p className="text-[9px] text-[#5d4037] uppercase font-black tracking-[0.4em] engraved">Classification</p>
                      <p className="text-sm text-[#3d2b1f] font-black italic drop-shadow-sm">{card.race} / {card.cardType}</p>
                    </div>

                    {/* Scrollable Description Box */}
                    <div className="flex-1 min-h-0 relative bg-black/[0.03] rounded-sm border border-black/5 shadow-inner overflow-hidden flex flex-col">
                      <div className="absolute top-0 right-0 p-2 opacity-5 rotate-12 pointer-events-none">
                        <Scroll size={40} className="text-[#3d2b1f]" />
                      </div>
                      <div className="p-3 overflow-y-auto h-full z-10 relative" style={{ scrollbarWidth: 'none' }}>
                        <p className="text-[11px] text-[#4e342e] leading-relaxed font-black italic drop-shadow-sm">"{card.description}"</p>
                      </div>
                    </div>
                  </div>

                  {/* ATK / DEF Bottom Section */}
                  {card.cardType !== 'Spell Card' && card.cardType !== 'Trap Card' && (
                    <div className="grid grid-cols-2 gap-3 mt-3 shrink-0">
                      <div className="bg-[#2a2a2a] p-2.5 rounded-sm border-b-[4px] border-black/60 flex justify-between items-center shadow-2xl active:translate-y-1 active:border-b-2 transition-all">
                        <Sword size={16} className="text-hearth-gold drop-shadow-md" />
                        <div className="text-right">
                          <p className="text-[8px] text-white/40 uppercase font-black tracking-widest">ATK</p>
                          <p className="text-xl text-white font-hearth font-black leading-none drop-shadow-md">{card.atk}</p>
                        </div>
                      </div>
                      {card.cardType !== 'Link Monster' && (
                        <div className="bg-[#2a2a2a] p-2.5 rounded-sm border-b-[4px] border-black/60 flex justify-between items-center shadow-2xl active:translate-y-1 active:border-b-2 transition-all">
                          <Shield size={16} className="text-white/60 drop-shadow-md" />
                          <div className="text-right">
                            <p className="text-[8px] text-white/40 uppercase font-black tracking-widest">DEF</p>
                            <p className="text-xl text-white font-hearth font-black leading-none drop-shadow-md">{card.def}</p>
                          </div>
                        </div>
                      )}
                    </div>
                ...
                <div className="absolute bottom-3 right-3 w-6 h-6 opacity-10 border-b-2 border-r-2 border-[#3d2b1f] pointer-events-none" />
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <button className="btn-stone-gold w-full !py-3.5 shadow-[0_15px_30px_rgba(244,208,63,0.3)] !text-sm font-black tracking-widest" onClick={onClose}>
